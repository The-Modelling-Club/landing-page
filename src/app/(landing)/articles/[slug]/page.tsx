...
export async function generateStaticParams() {
  ...
  const idRoutes = articles.map((article: ArticleInterface) => article.id);
...
export async function generateMetadata({
  ...
  const data: ArticleInterface = await client.fetch(findManuScriptById, {
    slug,
  });
  ...
  return {
    ...
    description:
      data.description ||
      `Read about ${data.title} on The Modeling Club's engineering insights page.`,
    keywords: `engineering, ${data.title.toLowerCase()}, computer-aided engineering, CAE, KNUST`,
    authors:
      data.authors?.map((author: AuthorInterface) => ({
    ...
    openGraph: {
      ...
      description:
        data.description ||
        `Read about ${data.title} on The Modeling Club's engineering insights page.`,
      url: `https://themodelingclub.com/articles/${data.id}`,
      type: "article",
      images: data.image_url
        ...
        : [],
      authors:
        data.authors?.map((author: AuthorInterface) => author.author_name) ||
        [],
    },
    twitter: {
